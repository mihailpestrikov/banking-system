...
public class CreditAccountTest {
    ...
    public void depositTest() throws InvalidRatioBetweenBordersAndInterests {
        ...
        var accountId = client.createAccount(AccountType.CreditAccount);
    ...
    @Test
    public void creditWithdraw() throws InvalidRatioBetweenBordersAndInterests, NotEnoughRightsToUseThisAccount, UnverifiedAccountLimit {
        Bank bank = CentralBank.getInstance().createBank(Setup.getBankParameters());
        var client = bank.clientBuilder.createClient("FirstName", "LastName").withAddress("qwe").withPassport("qwe").execute();
        var accountId = client.createAccount(AccountType.CreditAccount);
        ...
        client.withdraw(accountId, 1250.0);
        client.withdraw(accountId, 750.0);
        Double expected = -2200.0; // 200 - commission for negative balance operation
        Double actual = client.getAccount(accountId).getBalance();
    ...
    @Test
    public void failedCreditWithdrawAndLogsTest() throws InvalidRatioBetweenBordersAndInterests, NotEnoughRightsToUseThisAccount, UnverifiedAccountLimit {
        Bank bank = CentralBank.getInstance().createBank(Setup.getBankParameters());
        var client = bank.clientBuilder.createClient("FirstName", "LastName").withAddress("qwe").withPassport("qwe").execute();
        var accountId = client.createAccount(AccountType.CreditAccount);
